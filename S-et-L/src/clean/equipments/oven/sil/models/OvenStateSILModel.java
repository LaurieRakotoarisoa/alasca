...
public class OvenStateSILModel 
extends AtomicModel
implements SGMILModelImplementationI{
	...
	public OvenStateSILModel(String uri, TimeUnit simulatedTimeUnit, SimulatorI simulationEngine) throws Exception {
	...
	public static final String URI = OvenStateSILModel.class.getName();
	
	public static double DEFAULT_RATE = 0.1;
	public static double HIGH_RATE = 0.25;
		...
		) throws Exception
	{
		...
		this.componentRef =
			(Oven)
							simParams.get(OvenStateMILModel.COMPONENT_HOLDER_REF_PARAM_NAME) ;
	}
	...
	public void			userDefinedInternalTransition(Duration elapsedTime)
	{
		try {
			...
			double target = 
					(Double)this.componentRef.getEmbeddingComponentStateValue(OvenComponent.TARG_TEMP);
			if(target != this.targetTemperature) setTargetTemperature(target);
			assert target == this.targetTemperature;
			
			//get eco mode
			boolean eco = 
					(Boolean)this.componentRef.getEmbeddingComponentStateValue(OvenComponent.ECO_MODE);
			if(eco != this.ecoMode) setEcoMode(eco);
			assert eco == ecoMode;
			
			//get oven state
			OvenMode state = (OvenMode)this.componentRef.getEmbeddingComponentStateValue(OvenComponent.ECO_MODE);
			updateState(state);
	...
	public void updateState(OvenMode mode) {
		boolean oldDoor = doorOpened;
		boolean oldCompressorState = compressorActive;
		switch (mode) {
		case Off: this.doorOpened = false; 
						compressorActive = false;
						break;
		case On: this.doorOpened = false; 
